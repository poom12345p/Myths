...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
    ...
    private readonly Dictionary<int, ExitGames.Client.Photon.Hashtable> dataPerGroupReliable = new Dictionary<int, ExitGames.Client.Photon.Hashtable>();    // only used in RunViewUpdate()
    private readonly Dictionary<int, ExitGames.Client.Photon.Hashtable> dataPerGroupUnreliable = new Dictionary<int, ExitGames.Client.Photon.Hashtable>();  // only used in RunViewUpdate()
...
#region Operations and Connection Methods
    ...
    private void ReadoutProperties(ExitGames.Client.Photon.Hashtable gameProperties, ExitGames.Client.Photon.Hashtable pActorProperties, int targetActorNr)
    {
        ...
        if (pActorProperties != null && pActorProperties.Count > 0)
        {
            if (targetActorNr > 0)
            {
                ...
                if (target != null)
                {
                    ExitGames.Client.Photon.Hashtable props = this.ReadoutPropertiesForActorNr(pActorProperties, targetActorNr);
            ...
            else
            {
                ...
                {
                    actorNr = (int)key;
                    props = (ExitGames.Client.Photon.Hashtable)pActorProperties[key];
    ...
    internal protected bool SetMasterClient(int playerId, bool sync)
    {
        ...
        if (sync)
        {
            bool sent = this.OpRaiseEvent(PunEvent.AssignMaster, new ExitGames.Client.Photon.Hashtable() { { (byte)1, playerId } }, true, null);
            if (!sent)
            {
                return false;
            }
    ...
    {
        ExitGames.Client.Photon.Hashtable newProps = new ExitGames.Client.Photon.Hashtable() { { GamePropertyKey.MasterClientId, nextMasterId } };
        ExitGames.Client.Photon.Hashtable prevProps = new ExitGames.Client.Photon.Hashtable() { { GamePropertyKey.MasterClientId, this.mMasterClientId } };
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            case OperationCode.GetGameList:
                ...
                foreach (var gameKey in games.Keys)
                {
                    ...
                    this.mGameList[gameName] = new RoomInfo(gameName, (ExitGames.Client.Photon.Hashtable)games[gameKey]);
                }
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        switch (photonEvent.Code)
        {
            ...
            case EventCode.GameListUpdate:
                {
                    ...
                    foreach (var roomKey in games.Keys)
                    {
                        ...
                        RoomInfo game = new RoomInfo(gameName, (ExitGames.Client.Photon.Hashtable)games[roomKey]);
            ...
            case PunEvent.RPC:
                ...
                this.ExecuteRpc(photonEvent[ParameterCode.Data] as ExitGames.Client.Photon.Hashtable, originatingPlayer.ID);
                break;

            case PunEvent.SendSerialize:
            case PunEvent.SendSerializeReliable:
                ExitGames.Client.Photon.Hashtable serializeData = (ExitGames.Client.Photon.Hashtable)photonEvent[ParameterCode.Data];
            ...
            case PunEvent.Instantiation:
                this.DoInstantiate((ExitGames.Client.Photon.Hashtable)photonEvent[ParameterCode.Data], originatingPlayer, null);
                break;
            ...
            case PunEvent.DestroyPlayer:
                ExitGames.Client.Photon.Hashtable evData = (ExitGames.Client.Photon.Hashtable)photonEvent[ParameterCode.Data];
            ...
            case PunEvent.Destroy:
                evData = (ExitGames.Client.Photon.Hashtable)photonEvent[ParameterCode.Data];
    ...
    /// RPC ExitGames.Client.Photon.Hashtable Structure
    ...
    public void RunViewUpdate()
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            {
                ExitGames.Client.Photon.Hashtable groupHashtable = null;
                bool found = this.dataPerGroupReliable.TryGetValue(view.group, out groupHashtable);
                if (!found)
                {
                    groupHashtable = new ExitGames.Client.Photon.Hashtable(NetworkingPeer.ObjectsInOneUpdate);
        ...
        foreach (int groupId in this.dataPerGroupReliable.Keys)
        {
            ...
            ExitGames.Client.Photon.Hashtable groupHashtable = this.dataPerGroupReliable[groupId];
        ...
        {
            options.InterestGroup = (byte)groupId;
            ExitGames.Client.Photon.Hashtable groupHashtable = this.dataPerGroupUnreliable[groupId];
            if (groupHashtable.Count == 0)
            {
                continue;
            }
