...
public static class PhotonNetwork
{
    ...
    /// The whole ExitGames.Client.Photon.Hashtable will get sent. Minimize the traffic by setting only updated key/values.
    ///
    /// If the ExitGames.Client.Photon.Hashtable is null, the custom properties will be cleared.
    ...
    ///
    /// If customPropertiesToDelete is null or has 0 entries, all Custom Properties are deleted (replaced with a new ExitGames.Client.Photon.Hashtable).
    /// If you specify keys to remove, those will be removed from the ExitGames.Client.Photon.Hashtable but other keys are unaffected.
    /// </remarks>
    ...
    /// <param name="eventContent">Some serializable object like string, byte, integer, float (etc) and arrays of those. ExitGames.Client.Photon.Hashtables with byte keys are good to send variable content.</param>
    ...
    public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
    {
        ...
        ExitGames.Client.Photon.Hashtable instantiateEvent = networkingPeer.SendInstantiate(prefabName, position, rotation, group, viewIDs, data, false);
    ...
    public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
    {
        ...
        // Send to others, create info
        ExitGames.Client.Photon.Hashtable instantiateEvent = networkingPeer.SendInstantiate(prefabName, position, rotation, group, viewIDs, data, true);
